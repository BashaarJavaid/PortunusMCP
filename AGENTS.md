...
- `docker compose up -d` — development stack. Copy `.env.example` to `.env`, set the required `UPSTREAM_RUNTIME_NAMESPACE` and Docker-socket `DOCKER_GID`, and build `portunusmcp:dev` first.
...
**Phase 6 is active. Items 39–40 are complete; item 41 is next.** Local stdio upstreams are typed policy registry entries launched as one hardened Docker container per session (`services/gateway/upstream_client.py`, ADR-007). The `/mcp/*` edge now bounds body size/depth before auth, validates Host/Origin through the MCP SDK, admits only valid sessionless initialize requests, and applies per-identity session/in-flight/fixed-window call limits. Outstanding calls heartbeat their session and have a 60-second end-to-end deadline that reaps the session; `/ready` checks Postgres, Redis, and in-memory signing while `/health` remains liveness. Unknown Prometheus tool names use the bounded `other` label. `tests/integration/test_resource_limits.py`, the full suite, required real-Docker isolation, seven-beat demo, and N=1000 benchmark were re-run. This paragraph supersedes the historical Phase-5 “active” wording retained below.
