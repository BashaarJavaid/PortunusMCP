...
**Phase 6 is active. Items 39–41 are complete; item 42 is next.** Local stdio upstreams run in one hardened container per session (ADR-007); the HTTP/session/call lifecycle is bounded and `/ready` is dependency-aware. The old implicit development deployment is now explicit `compose.demo.yml`; production is the hardened, digest-pinned, single-replica `compose.prod.yml` with internal authenticated Postgres/Redis, one-shot migrations, separate gateway secrets, no rogue service, and secured optional monitoring. Multi-replica operation remains unsafe and deferred. Item 41 verification: production Compose smoke passed end to end; full suite 377 passed at 92.12% coverage; required real-Docker isolation, Ruff, format, and mypy passed. This paragraph supersedes the historical Phase-5 “active” wording retained below.
