...
**Phase 6 is active. Item 39 is complete; item 40 is next.** Local stdio upstreams are now typed policy registry entries launched as one hardened Docker container per session (`services/gateway/upstream_client.py`, ADR-007): minimal allowlisted environment, no gateway secrets or Docker socket, non-root/read-only/no-capabilities launch, no network by default, resource limits, startup/reload/rollback preflight, runtime-fingerprint eviction, and orphan/reap handling. The verifier mounts only the public key. `tests/integration/test_upstream_isolation.py` is the required real-Docker boundary/resource check; the full live demo and N=1000 real-container benchmark were re-run. This paragraph supersedes the historical Phase-5 “active” wording retained below.
