...
**Phase 6 is active. Items 39–42 are complete; item 43 is next.** Local stdio upstreams run in one hardened container per session (ADR-007); the HTTP/session/call lifecycle is bounded and `/ready` is dependency-aware. The old implicit development deployment is now explicit `compose.demo.yml`; production is the hardened, digest-pinned, single-replica `compose.prod.yml` with internal authenticated Postgres/Redis, one-shot migrations, no rogue service, secured optional monitoring, and writable operator-owned policy/audit-key roots. Item 42 adds the installed stdlib `portunusmcp` CLI, bounded operator queues and candidate-policy APIs, durable journaled policy rollout/rollback/SIGHUP, fingerprinted audit-key rotation, and verified self-contained audit export (ADR-008); the distributed writer remains deferred. Item 42 verification: installed CLI live approval + production baseline/export smoke passed; full suite 381 passed at 88.44% coverage; required real-Docker isolation, Ruff, format, and strict mypy passed. This paragraph supersedes the historical Phase-5 “active” wording retained below.
